//
//  TestIfPlayerEntersCorrectGuessWinsTheGame.swift
//  GuessGameUnitTests
//
//  Created by Pawel Kijowski on 4/23/20.
...
final class TestIfPlayerEntersCorrectGuessWinsTheGame: XCTestCase, GuessGameDelegate {
    var game: GuessGame!
    var players: [Player]!
    ...
    func handle(event: GameEvent) {
        ...
        if event.type == .readyForUserInput {
            ...
            if p.name == "Eva" {
                ...
                guard let nextPlayerInputCommand = factory.makeCommand(params:["player":p.name,"type":"PlayerInputCommand","value":50]) else { XCTFail(); return }
    ...
    func test() {
        ...
        let factory = EngineCommandFactory()
        guard let startGameCommand = factory.makeCommand(params: ["players":players!,"type":"ConfigureGameCommand","range":(0...100),"numberOfGuessesPerPlayer":1,"winningGuess":50,"delay":1.0]) else { XCTFail(); return }
