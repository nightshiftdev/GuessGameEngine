//
//  TestWhenPlayerInputCmdIsSentTheEventCarriesAHint.swift
//  GuessGameUnitTests
//
//  Created by Pawel Kijowski on 4/18/20.
...
final class TestWhenPlayerInputCmdIsSentTheEventCarriesAHint: XCTestCase, GuessGameDelegate {
    var game: GuessGame!
    var players: [Player]!
    var playerIdx = 0
    ...
    func test() {
        ...
        let factory = EngineCommandFactory()
        guard let startGameCommand = factory.makeCommand(params: ["players":players!,"type":"ConfigureGameCommand","range":(0...100),"numberOfGuessesPerPlayer":1,"delay":2.0,"winningGuess":50]) else { XCTFail(); return }
